...
def normalize_ohlc_data(
        open: pd.Series, high: pd.Series, low: pd.Series,
...
def generate_random_crypto_df(days: int, start_price: float, col_name: str,
                              start_date: str,
    ...
    return pd.DataFrame({
        ...
        'date':
        np.tile(pd.date_range(start_date, periods=periods, freq='H'), 1),
        'price': (price)
    })
...
def real_case(df: pd.DataFrame, start_date: str,
...
def random_case(num_days: int, start_price: float, asset_name: str,
    ...
    df = generate_random_crypto_df(num_days,
                                   start_price,
                                   asset_name,
                                   start_date,
...
def create_figure(index: pd.RangeIndex,
                  open: pd.Series,
                  high: pd.Series,
                  low: pd.Series,
                  close: pd.Series,
                  answer=None) -> go.Figure:
...
def main() -> None:
    ...
    for i in range(total_graphs):
        ...
        fig.write_html(file=f"html/HABC-USD_{i}.html",
                       config=get_config(),
                       include_plotlyjs=True,
                       validate=True)
        fig.show(config=get_config())

        fig = create_figure(df.index, norm_open, norm_high, norm_low,
                            norm_close, f'FABC/USD {i}')
        fig.write_html(file=f"html/FABC-USD_{i}.html",
                       config=get_config(),
                       include_plotlyjs=True,
                       validate=True)
        fig.show(config=get_config())
