...
class RandomOHLC:
    ...
    def __generate_random_df(
        ...
        # if the number of bars does not go out of index range of dataframe
        # vol_period_num_bars < len(self.__df), # put this check in the function
    ...
    def __create_volatile_periods(self) -> None:
        ...
        for i in tqdm(range(len(self.__df))):
            if random.randint(1, VOLATILE_PROB) < random_chance:
                vol_period_num_bars = random.randint(
                    VOLATILE_PERIOD_MIN, VOLATILE_PERIOD_MAX
                )

                vec_arr = np.where(
                    vol_period_num_bars < len(self.__df) - i,
                    self.__generate_random_df(
                ...
                for j, row in enumerate(vec_arr):
                    self.__df.at[i + j, 'open'] = row[0]
                    self.__df.at[i + j, 'high'] = row[1]
                    self.__df.at[i + j, 'low'] = row[2]
                    self.__df.at[i + j, 'close'] = row[3]
